            ...
            if(password_verify($password, $hashed_password)){
                ...
                $_SESSION['lastname'] = $lastname;
                                ...
                                if(isset($_SESSION['userID'])){
                                        ...
                                        $sqlQuery = "SELECT * FROM perms WHERE userID = :userID AND accountID = :accountID";
                                        $statement = $db->prepare($sqlQuery);
                                        $statement->execute(array(':userID' => $userID, ':accountID' => $accountID));
        
        
                                        while($row = $statement->fetch()){
        
                                        $canEditAgents = $row['canEditAgents'];
                                        if($canEditAgents == 1) {$_SESSION['canEditAgents'] = $canEditAgents;}
                                        
                                        $isExpoAgent = $row['isExpoAgent'];
                                        if($isExpoAgent == 1) {$_SESSION['isExpoAgent'] = $isExpoAgent;}
                ...
                if(isset($_SESSION['isExpoAgent'])) {
                     header('location: /inc/page/expoAgents/redirectNewEnteryWithIdentifier.php');
                } else {
